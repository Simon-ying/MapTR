    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        if 'imgs' in inputs:
            ...
            if data_samples is not None:
                ...
                for queue_id in range(self.queue_length):
                    for data_sample, pad_shape in zip(data_samples[queue_id],
                                                    batch_pad_shape):
                    ...
                    if self.boxtype2tensor:
                        samplelist_boxtype2tensor(data_samples[queue_id])
                    if self.pad_mask:
                        self.pad_gt_masks(data_samples[queue_id])
                    if self.pad_seg:
                        self.pad_gt_sem_seg(data_samples[queue_id])
